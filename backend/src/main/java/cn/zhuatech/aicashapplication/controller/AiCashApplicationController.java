...
import cn.zhuatech.aicashapplication.model.*;
import cn.zhuatech.aicashapplication.service.AiCashApplicationService;
...
public class AiCashApplicationController {
    private final AiCashApplicationService service; public AiCashApplicationController(AiCashApplicationService service){this.service=service;}
    @GetMapping("/public/about") ApiResponse<Map<String,Object>> about(){return ApiResponse.ok(service.about());}
    @GetMapping("/catalog") ApiResponse<AiCashApplicationService.CatalogView> catalog(){return ApiResponse.ok(service.catalog());}
    @GetMapping("/dashboard") ApiResponse<AiCashApplicationService.Dashboard> dashboard(){return ApiResponse.ok(service.dashboard());}
    ...
    @PostMapping("/records/{id}/comments") ApiResponse<BusinessRecord> comment(@PathVariable Long id,@Valid @RequestBody AiCashApplicationService.CommentRequest request){return ApiResponse.ok(service.comment(id,request));}
    @GetMapping("/sla-summary") ApiResponse<AiCashApplicationService.SlaSummary> sla(){return ApiResponse.ok(service.slaSummary());}
    @PostMapping("/records") ApiResponse<BusinessRecord> create(@Valid @RequestBody AiCashApplicationService.RecordRequest request){return ApiResponse.ok(service.create(request));}
    @PutMapping("/records/{id}") ApiResponse<BusinessRecord> update(@PathVariable Long id,@Valid @RequestBody AiCashApplicationService.RecordRequest request){return ApiResponse.ok(service.update(id,request));}
    @PostMapping("/records/{id}/actions") ApiResponse<BusinessRecord> action(@PathVariable Long id,@Valid @RequestBody AiCashApplicationService.ActionRequest request){return ApiResponse.ok(service.action(id,request));}
